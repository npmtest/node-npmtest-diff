...
<html lang="en">
<head>
    <title>Code coverage report for node-npmtest-diff/node_modules/diff/lib/diff/word.js</title>
    <meta charset="utf-8">
    <style>
        body, html {
            margin:0; padding: 0;
...
<div class="header medium">
    ...
    <h1>Code coverage report for <span class="entity">node-npmtest-diff/node_modules/diff/lib/diff/word.js</span></h1>
    <h2>
        ...
        Lines: <span class="metric">57.69% <small>(15 / 26)</small></span> &nbsp;&nbsp;&nbsp;&nbsp;
        
        Ignored: <span class="metric"><span class="ignore-none">none</span></span> &nbsp;&nbsp;&nbsp;&nbsp;
    </h2>
    <div class="path"><a href="../../../../../index.html">All files</a> &#187; <a href="index.html">node-npmtest-diff/node_modules/diff/lib/diff/</a> &#187; word.js</div>
</div>
<div class="body">
...
wordDiff.equals = <span class="fstat-no" title="function not covered" >function (left, right) {</span>
<span class="cstat-no" title="statement not covered" >  return left === right || this.options.ignoreWhitespace &amp;&amp; !reWhitespace.test(left) &amp;&amp; !reWhitespace.test(right);</span>
};
wordDiff.tokenize = <span class="fstat-no" title="function not covered" >function (value) {</span>
<span class="cstat-no" title="statement not covered" >  var tokens = value.split(/(\s+|\b)/);</span>
&nbsp;
  // Join the boundary splits that we do not consider to be boundaries. This is primarily the extended Latin character set.
<span class="cstat-no" title="statement not covered" >  for (var i = 0; i &lt; tokens.length - 1; i++) {</span>
    // If we have an empty string in the next field and we have only word chars before and after, merge
<span class="cstat-no" title="statement not covered" >    if (!tokens[i + 1] &amp;&amp; tokens[i + 2] &amp;&amp; extendedWordChars.test(tokens[i]) &amp;&amp; extendedWordChars.test(tokens[i + 2])) {</span>
<span class="cstat-no" title="statement not covered" >      tokens[i] += tokens[i + 2];</span>
...
&nbsp;
<span class="fstat-no" title="function not covered" >function diffWords(oldStr, newStr, callback) {</span>
<span class="cstat-no" title="statement not covered" >  var options = /*istanbul ignore start*/(0, _params.generateOptions) /*istanbul ignore end*/(callback, { ignoreWhitespace: true });</span>
<span class="cstat-no" title="statement not covered" >  return wordDiff.diff(oldStr, newStr, options);</span>
}
<span class="fstat-no" title="function not covered" >function diffWordsWithSpace(oldStr, newStr, callback) {</span>
<span class="cstat-no" title="statement not covered" >  return wordDiff.diff(oldStr, newStr, callback);</span>
}
